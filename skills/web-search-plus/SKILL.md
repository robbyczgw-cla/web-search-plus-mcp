...
4. If `web_search` returns an error that no provider is configured, tell the
   user to run `uvx --from web-search-plus-mcp==4.3.1 web-search-plus-mcp status`
...
```bash
uvx --from web-search-plus-mcp==4.3.1 web-search-plus-mcp status
```

Write a starter `.env` template:

```bash
uvx --from web-search-plus-mcp==4.3.1 web-search-plus-mcp setup --preset starter
```
